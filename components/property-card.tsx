...
export function PropertyCard({ property, onClick }: PropertyCardProps) {
  const whatsappMessage = encodeURIComponent(
    `Hola, estoy interesado en la propiedad "${property.nombre}" ubicada en ${property.direccion}. ¿Podrían darme más información?`,
  )
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="p-4">
        <h3 className="font-semibold text-lg text-card-foreground mb-2 line-clamp-1">{property.nombre}</h3>
        ...
        <p className="text-sm text-muted-foreground line-clamp-2 mb-4">{property.descripcion}</p>

        <div className="flex items-center justify-between">
          <span className="text-lg font-bold text-secondary">{formatPrice(property.precio)}</span>
        </div>

        <Button
          onClick={(e) => {
            e.stopPropagation()
            window.open(`https://wa.me/573117284320?text=${whatsappMessage}`, "_blank")
          }}
